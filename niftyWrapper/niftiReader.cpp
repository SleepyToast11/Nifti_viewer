//Code taken and modified from : https://github.com/NIFTI-Imaging/nifti_clib/
//as in accordance with the licence as of (May 23 2024):
...
int nifti_compiled_with_zlib(void)
{
    ...
    return 0; //this is if not compiled with zlib
                         ...
                         int comp)
{
 ...
 /* if compression is requested, make sure of suffix */
...
void * nifti_read_header( const char *hname, int *nver, int check )
{
    ...
    if( ni_ver == 0 || ni_ver == 1 ) {
        ...
        if ( check && ! nifti_hdr1_looks_good((const nifti_1_header*)hresult) ){
            LNI_FERR(fname,"nifti_1_header looks bad for file", hname);
            return hresult;
        }
    } else if ( ni_ver == 2 ) {
        ...
        if ( check && ! nifti_hdr2_looks_good((const nifti_2_header*)hresult) ){
            LNI_FERR(fname,"nifti_2_header looks bad for file", hname);
            return hresult;
        }
    } else {
